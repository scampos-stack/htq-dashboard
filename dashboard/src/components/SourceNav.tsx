...
const GROUPS = [
  {
    label: null,
    sources: [{ value: "all", label: "All Sources", dot: "bg-charcoal" }],
  },
  {
  ...
  {
    label: "Customer",
    sources: [{ value: "zendesk", label: "Zendesk", dot: "bg-teal-500" }],
  },
];
...
export function SourceNav() {
  ...
  return (
    <nav className="sticky top-6 flex shrink-0 flex-col gap-4 self-start sm:w-52">
      {GROUPS.map((group, i) => (
        <div key={group.label ?? `group-${i}`} className="flex flex-col gap-1">
          {group.label && (
            <span className="px-4 pb-1 text-xs font-semibold uppercase tracking-wide text-body-gray">
              {group.label}
            </span>
          )}
          {group.sources.map((s) => {
